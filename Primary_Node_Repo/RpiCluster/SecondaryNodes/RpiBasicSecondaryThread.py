...
class RpiBasicSecondaryThread(threading.Thread):
    ...
    def __init__(self, primary_ip, primary_port):
        ...
        self.uuid = None
        self.myID = None
        self.NbrInfo = {} # list of out-neighbors' (including myself) addresses 
        ...
        self.Diam = 5;  # can be passed as an input in a later update
    ...
    def getNbrConnections(self):
        ...
        while True:         
            ...
            if message['type'] == 'NbrInfo':
                ...
                # logger.info(message)
                self.NbrInfo = message['payload'] # We consider out-neighbors here
                logger.info("my Nbr addresses")              
                logger.info(self.NbrInfo)
                break
    ...
    def broadcastMsgto_OUT_Nbrs(self):
        ...
        myUpdate = json.dumps({"numerator": n, "denominator": d, "max": float(self.myMax), "min": float(self.myMin)})
        
        # start making connections with the neighbors                
        self.my_listening_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.my_listening_socket.bind((self.my_listening_ip, self.my_listening_port))  
        # logger.info("ready to connect to nbrs")
        self.my_listening_socket.listen(len(self.NbrInfo))  # listen to NbrInfo number of neighbor node-connects
        
        # Sockets to send messages to 
        for Nbr in range(len(self.NbrInfo)):
            with socket.socket() as Nbr_socket:
                try:                    
                    Nbr_address = tuple(self.NbrInfo[Nbr])
                    ...
                    logger.info("Failed to connect to my Nbr, waiting 1 seconds and trying again")
                    time.sleep(1)       
    ...
    def receiveMsgfrom_IN_Nbrs(self):
        ...
        while True:
            ...
            if data['type'] == 'myUpdate':   
               ...
               # self.IN_neighborlocalFlags = np.concatenate((np.array(self.IN_neighborlocalFlags), np.array(dummy['local_flag'])),axis=None)
               # self.IN_neighborglobalFlags = np.concatenate((np.array(self.IN_neighborglobalFlags), np.array(dummy['global_flag'])),axis=None)
               # logger.info("self.IN_neighborNums = " +str(self.IN_neighborNums))
               # logger.info("self.IN_neighborDens = " +str(self.IN_neighborDens))
               # logger.info("self.IN_neighborMaxs = " +str(self.IN_neighborMaxs))
               # logger.info("self.IN_neighborMins = " +str(self.IN_neighborMins))
               # logger.info("self.IN_neighborlocalFlags = " +str(self.IN_neighborlocalFlags))
               # logger.info("self.IN_neighborglobalFlags = " +str(self.IN_neighborglobalFlags))
               
   
            ii += 1;
           
            if (ii == len(self.NbrInfo)):
               # self.Nbrsflag = np.array(self.IN_neighborlocalFlags).min()            
               # self.global_flag = np.array(self.IN_neighborglobalFlags).min()
               # self.conv_flag = np.minimum(self.myflag, self.Nbrsflag)
               # logger.info("I updated my info based on my in-neighbors")  
               break
        ...
        for Nbr in range(len(self.NbrInfo)):
            with socket.socket() as Nbr_socket:
                try:                    
                    ...
                    l_onoff = 1                                                                                                                                                           
                    l_linger = 0                                                                                                                                                          
                    Nbr_socket.setsockopt(socket.SOL_SOCKET, socket.SO_LINGER, struct.pack('ii', l_onoff, l_linger))
                    # send data here
                    Nbr_socket.close()
                except socket.error as e:
                    logger.info("Failed to connect to my Nbr, waiting 1 seconds and trying again")
    ...
    # def epsilonconsUpdate(self, initialVal, consTol):  
        
        
    #     outDeg = len(self.NbrInfo)
    #     weight = 1/(outDeg + 1)
        
    #     self.myNum = initialVal
    #     self.myDen = 1
    #     self.myRatio = self.myNum
    #     self.myMax = initialVal
    #     self.myMin = initialVal
    #     self.myflag = 0
    #     self.Nbrsflag = 0
    #     self.global_flag = 0
    #     self.conv_flag = 0
    ...
    #         # if ((self.conv_flag and self.global_flag) == 1):
    #         if (self.conv_flag == 1):
    #             # logger.info("looks like consensus is reached")
    #             break;
    ...
    def epsilonconsUpdate(self, initialVal, consTol):  
        ...
        while True:    
            ...
            # self.mybitFlag =  np.minimum(self.mybitFlag, np.array(self.IN_neighborlocalFlags).min())
    ...
    def result_to_primary(self, converged_value):
        ...
        self.connection_handler.send_message(converged_value, "converged")


   
    def receive_latestMsg_from_Primary(self):
        message = self.connection_handler.get_message()
        isNewMsg = 0  
        ...
        if message['type'] == 'latest_parameters':
            ...
            if abs( message['payload'] > 0):
                isNewMsg = 1
                if ( isinstance(self.functionInfo, str) == False):
                    if abs(self.functionInfo - message['payload']) < 2:
                        isNewMsg = 0  
                    else:
                        self.functionInfo = message['payload']
                else:
                    self.functionInfo = message['payload']
            # logger.info("my latest function parameter =" + str(self.functionInfo))
        return isNewMsg
