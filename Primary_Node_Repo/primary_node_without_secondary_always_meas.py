...
def get_updates_from_secondary():  
    ...
    while threadGo:    
        ...
        if (flag == NumNodes):
            ...
            # Thread(target=primary.send_stop_to_agents()).start()
...
def receive_meas_OPAL():
    ...
    while threadGo:        
        ...
        # average = np.sum(measVec)/NumNodes
        # # logger.info("sending new updates to all secondaries= " +str(newVec))
        # # primary.send_latest_updated_info_secondary(newVec)
        
        # dummy = np.zeros(NumNodes)
        # for k in range(NumNodes):
        #     dummy[k] = (0.5*average - measVec[k]); 
        # prevAdj = dummy
        # send_dispatch_to_OPAL(dummy)  
